...
#         df = pd.read_csv("src/corrected_csv.csv")
        
#         # Ensure required columns exist
#         required_cols = ['title', 'price', 'beds', 'address', 'lat', 'lon', 'link', 'neighborhood', 'is_furnished']
#         for col in required_cols:
#             if col not in df.columns:
#                 st.error(f"Missing required column in CSV: '{col}'. Please check your 'corrected_deepseek_rentals.csv' format.")
#                 return None
...
#         # Note: For very large datasets, pre-calculating this during the scraping/data prep phase
#         # and saving it to the CSV would be more performant than doing it here on app load.
#         df['distance_to_su_miles'] = df.apply(
#             lambda row: geodesic((row['lat'], row['lon']), SU_COORDS).miles,
#             axis=1
#         )
        
#         return df
#     except FileNotFoundError:
#         st.error("Error: 'corrected_deepseek_rentals.csv' not found. Please ensure it's in the same directory as your Streamlit app.")
#         return None
#     except Exception as e:
#         st.error(f"Error loading or processing data: {str(e)}. Please check your CSV file content.")
#         return None
...
#     # Convert 'Int64' to 'int' for unique() if not already done, then to string for display
...
#     try:
#         # Load the corrected CSV file
#         file_path =( "src/corrected_csv.csv")
        
#         # Check if the file exists before attempting to read
#         import os
#         if not os.path.exists(file_path):
#             st.error(f"Error: File not found at '{file_path}'. Please check your file path and folder structure.")
#             st.write("Current working directory:", os.getcwd())
#             st.write("Expected file path:", os.path.abspath(file_path))
#             return None

#         df = pd.read_csv(file_path)
#         st.write(f"Successfully loaded '{file_path}'. Initial shape: {df.shape}")
#         st.write("Initial DataFrame head:")
...
#         for col in required_cols:
#             if col not in df.columns:
#                 st.error(f"Missing required column in CSV: '{col}'. Please ensure your CSV has all these columns.")
...
#         # st.write(f"Shape after type conversion: {df.shape}")
#         # st.write("DataFrame info after type conversion:")
#         st.text(df.info()) # Use st.text to display df.info()
...
#         # st.write(f"Shape after processing 'is_furnished': {df.shape}")

#         # Drop rows with any missing crucial data for map/filters
#         df_before_dropna = df.shape[0]
#         df = df.dropna(subset=['price', 'beds', 'lat', 'lon', 'address'])
#         # st.write(f"Shape after dropping rows with missing crucial data: {df.shape} (Dropped {df_before_dropna - df.shape[0]} rows)")
        
#         if df.empty:
#             st.warning("DataFrame is empty after processing. This might be due to all rows having missing data in crucial columns or incorrect data types.")
#             return None
...
#             axis=1
#         )
#         # st.write(f"Final DataFrame shape after distance calculation: {df.shape}")
#         # st.write("Final DataFrame head (with distance):")
#         st.dataframe(df.head())

#         return df
#     except FileNotFoundError:
#         st.error(f"Error: The file '{file_path}' was not found. Please ensure it's in the correct 'data' subfolder or adjust the path.")
#         return None
#     except Exception as e:
#         st.error(f"An unexpected error occurred during data loading or processing: {str(e)}")
#         import traceback
#         st.exception(e) # Display full traceback for more details
#         return None
#     finally:
#         st.write("--- End Debugging Data Loading ---")
...
#         with col1:
#             st.write("### Map View")
#             # Create a base map with SU location for context
#             st.map(filtered_df[['lat', 'lon']].assign(
#                 size=200, # Size of markers
#                 color=[[255, 0, 0, 160]] * len(filtered_df) # Red markers for rentals
#             ))
#             # You could add a custom marker for SU itself if using pydeck/folium for more control
...
#         # Display clickable links or expanded details
...
# else:
#     st.warning("Data could not be loaded or is empty after processing. Please check the debug information above.")
